...
class ARCroco3DStereo(CroCoNet):
    ...
    def _encode_views(self, views, img_mask=None, ray_mask=None):
        ...
        for view in views:
            if "true_shape" in view:
                ...
                shape = torch.tensor(view["img"].shape[1:], device=device)
    ...
    def _forward_decoder_step(
        ...
        update_mask = update_mask[:, None, None].float().to(new_state_feat.device)
        ...
        if reset_mask is not None:
            reset_mask = reset_mask[:, None, None].float().to(new_state_feat.device)
